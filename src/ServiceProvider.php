...
class ServiceProvider extends AddonServiceProvider
{
    ...
    // Register routes before other components
    public function register()
    {
        parent::register();
        
        // Force early route registration
        if (isset($this->routes['cp'])) {
            $this->loadRoutesFrom($this->routes['cp']);
        }
    }

    public function bootAddon()
    {
        // Make sure routes are registered before navigation
        $this->bootPermissions();
        $this->publishPartial();
        
        // Register navigation last, after routes are available
        $this->bootNavigation();
    }
    
    // We're not overriding registerCpRoutes anymore as it's already defined in parent
